...
function Skeleton() {
  return (
    <div className="flex items-center gap-3 p-4 rounded-card" style={{ backgroundColor: '#111115' }}>
      <div className="w-10 h-10 rounded-full shrink-0 animate-pulse" style={{ backgroundColor: 'rgba(255,255,255,0.07)' }} />
      <div className="flex-1 space-y-2">
        <div className="h-3.5 w-2/3 rounded-full animate-pulse" style={{ backgroundColor: 'rgba(255,255,255,0.07)' }} />
        <div className="h-2.5 w-1/3 rounded-full animate-pulse" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }} />
      </div>
      <div className="h-4 w-16 rounded-full animate-pulse" style={{ backgroundColor: 'rgba(255,255,255,0.07)' }} />
    </div>
  )
}
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="flex flex-col items-center py-20 gap-5 px-8">
      ...
      >
        <Receipt size={32} color="#C4A86A" strokeWidth={1.5} />
      </div>
      <div className="text-center space-y-1.5">
        <p style={{ fontSize: 16, fontWeight: 700, color: '#EDE9E0' }}>Sin gastos aún</p>
        <p style={{ fontSize: 14, color: '#7A7A84' }}>Agrega tu primer gasto para empezar a registrar el viaje.</p>
      </div>
      <button
        type="button"
        onClick={onAdd}
...
function SwipeRow({ expense, onDelete }: { expense: Expense; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="relative overflow-hidden" style={{ borderRadius: 0 }}>
      <div className="absolute inset-0 flex items-center justify-end pr-5" style={{ backgroundColor: '#C84040' }}>
      ...
      <motion.div
        ...
        style={{ backgroundColor: '#111115', touchAction: 'pan-y' }}
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: '#EDE9E0' }} className="truncate">
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <p style={{ fontSize: 15, fontWeight: 800, color: '#EDE9E0' }}>
            {formatCC(expense.amount, expense.currency)}
          </p>
          <p style={{ fontSize: 11, color: '#7A7A84', marginTop: 2 }}>{expense.currency}</p>
        </div>
      </motion.div>
    </div>
  )
...
export function ExpenseListScreen({ trip }: { trip: Trip }) {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#09090B' }}>
      {/* ── Hero image ──────────────────────────── */}
      <div className="relative" style={{ height: 200, backgroundColor: '#111115' }}>
        <img src={TRIP_IMAGE} alt={trip.name} className="w-full h-full object-cover" loading="lazy" />
        <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(to top, #09090B 0%, rgba(9,9,11,0.4) 60%, transparent 100%)' }}
        />
        <div className="absolute bottom-0 left-0 right-0 px-5 pb-5">
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)' }}>{trip.destination ?? 'Sin destino'}</p>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: '#EDE9E0', margin: '2px 0 0', lineHeight: 1.1 }}>
            {trip.name}
          </h1>
        </div>
      </div>
      ...
      <div className="px-5 pt-4 pb-5 space-y-3">
        ...
        <div
          style={{
            background: 'linear-gradient(135deg, rgba(196,168,106,0.1) 0%, rgba(27,98,122,0.06) 100%)',
            border: '1px solid rgba(196,168,106,0.15)',
            borderRadius: 22,
            padding: 20,
          }}
        >
          <p style={{ fontSize: 11, fontWeight: 700, color: '#7A7A84', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>
            Total gastado
          </p>
          <p style={{ fontSize: 38, fontWeight: 800, color: '#EDE9E0', lineHeight: 1, letterSpacing: '-0.5px' }}>
            {formatCC(grandTotal, primaryCurrency)}
          </p>
          <p style={{ fontSize: 13, color: '#7A7A84', marginTop: 6, marginBottom: 12 }}>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span style={{ fontSize: 11, color: '#7A7A84' }}>Presupuesto usado</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: budgetUsedPct > 0.8 ? '#C84040' : '#C4A86A' }}>
            ...
            <div style={{ height: 5, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}>
              <motion.div
                ...
                style={{
                  ...
                  background: budgetUsedPct > 0.8
                    ? 'linear-gradient(90deg, #C84040, #E05252)'
                    : 'linear-gradient(90deg, #C4A86A, #D4BA7C)',
                }}
              />
            </div>
          </div>
      ...
      <div className="px-5 mb-4">
        ...
        >
          ...
          >
            <Sparkles size={16} color="#C4A86A" />
          </div>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: '#C4A86A', marginBottom: 2 }}>
              {AI_INSIGHTS[insightIdx]!.icon} Insight IA
            </p>
            <p style={{ fontSize: 13, color: '#EDE9E0', lineHeight: 1.45 }}>
          ...
          <button
            ...
            style={{ color: '#C4A86A', fontSize: 18, flexShrink: 0, paddingLeft: 4 }}
          >
            →
          </button>
        </motion.div>
          ...
          <div className="flex gap-3 px-5 overflow-x-auto no-scrollbar">
            {Array.from(categoryTotals.entries()).map(([cat, total]) => {
              ...
              return (
                <div
                  key={cat}
                  className="flex flex-col items-center gap-2 p-3 rounded-card shrink-0"
                  style={{
                ...
                >
                  ...
                  <p style={{ fontSize: 11, fontWeight: 800, color: '#EDE9E0', textAlign: 'center' }}>
          ...
          <AnimatePresence mode="popLayout">
            {sortedDates.map((date) => (
              <motion.div
                ...
                className="space-y-2"
              >
                <p style={{ fontSize: 11, fontWeight: 700, color: '#4A4A52', textTransform: 'uppercase', letterSpacing: '0.08em', paddingLeft: 4 }}>
                  {dateLabel(date)}
                </p>
                <div
                  style={{
